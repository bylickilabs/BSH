...
## 2. Mathematische Formeln und Prozesse

### 2.1 Asymmetrische Schlüsselerzeugung (Elliptic Curve Cryptography - ECC)

- **Privater Schlüssel \( d \in \mathbb{Z}_p \)**  
  Ein zufällig gewählter Wert aus dem endlichen Feld \(\mathbb{Z}_p\), wobei \(p\) eine große Primzahl (für Curve25519 ist das \(p = 2^{255} - 19\)) ist.

- **Öffentlicher Schlüssel \( Q \)**  
  Der öffentliche Schlüssel wird durch Multiplikation des privaten Schlüssels \(d\) mit dem Generatorpunkt \(G\) auf der elliptischen Kurve gebildet:  
  \[
  Q = d \times G
  \]  
  Diese Operation ist auf elliptischen Kurven definiert und gewährleistet, dass der private Schlüssel nicht aus dem öffentlichen Schlüssel abgeleitet werden kann (Elliptic Curve Discrete Logarithm Problem).

**Erklärung:**  
Diese asymmetrische Schlüsselerzeugung ermöglicht es, sichere Schlüsselpaare zu generieren, die für den Schlüsselaustausch verwendet werden.

---

### 2.2 Sitzungsschlüssel (Symmetrisch) via Diffie-Hellman

- **Berechnung des gemeinsamen geheimen Schlüssels \( S \)**  
  Zwei Kommunikationspartner mit privaten Schlüsseln \( d_A \) und \( d_B \) und öffentlichen Schlüsseln \( Q_A \) und \( Q_B \) berechnen unabhängig voneinander:  
  \[
  S = d_A \times Q_B = d_B \times Q_A
  \]  
  Aufgrund der Eigenschaften der elliptischen Kurve resultiert in beiden Fällen derselbe Punkt \(S\).

- **Schlüsselableitung mittels HKDF:**  
  Der Punkt \( S \) wird als Input für die Hash-basierte Schlüsselableitung verwendet, um einen symmetrischen Sitzungsschlüssel zu generieren:  
  \[
  K_{session} = \mathrm{HKDF}(S, \mathrm{info})
  \]  
  HKDF stellt sicher, dass der Schlüssel sicher und zufällig verteilt ist.

**Erklärung:**  
Dieser Prozess erlaubt es beiden Parteien, ohne direkten Schlüsselaustausch einen gemeinsamen Schlüssel zu vereinbaren.

---

### 2.3 Verschlüsselung mit AES-256 im Galois/Counter Mode (GCM)

- **Klartext \( M \)**  
  Die zu verschlüsselnden Daten.

- **Nonce \( N \)**  
  Ein einmaliger, zufälliger Wert (96 Bit), der für die Sicherheit des GCM-Modus essentiell ist.

- **Verschlüsselungs- und Authentifizierungsprozess:**  
  \[
  (C, T) = \mathrm{AES\_GCM\_Encrypt}(K_{session}, N, M)
  \]  
  Hierbei ist \(C\) der verschlüsselte Chiffretext und \(T\) der Authentifizierungs-Tag, der die Integrität und Authentizität sicherstellt.

**Erklärung:**  
AES-GCM bietet eine effiziente Verschlüsselung mit gleichzeitiger Integritätsprüfung der Daten.

---

### 2.4 Schlüsselrotation

- **Neuer Schlüssel \( K_{new} \)**  
  Um die Sicherheit zu erhöhen, wird der symmetrische Schlüssel periodisch erneuert. Dabei wird der alte Schlüssel zusammen mit einem Zeitstempel \( t \) in die HKDF-Funktion gegeben:  
  \[
  K_{new} = \mathrm{HKDF}(K_{old}, \mathrm{info} \parallel t)
  \]

- \( \parallel \) steht für die Verkettung von Daten.

**Erklärung:**  
Diese Rotation reduziert das Risiko, das von kompromittierten Schlüsseln ausgeht, da Schlüssel nur zeitlich begrenzt gültig sind.

---

## Diagrammbeschreibungen (zur Visualisierung)

1. **Schlüsselerzeugung und Austausch:**  
   Zwei Benutzer erzeugen jeweils ein Schlüsselpaar (privat/public). Sie tauschen ihre öffentlichen Schlüssel aus und berechnen daraus den gemeinsamen geheimen Schlüssel \( S \).

2. **Sitzungsschlüssel-Ableitung:**  
   Aus \( S \) wird über HKDF der symmetrische Sitzungsschlüssel \( K_{session} \) abgeleitet.

3. **Verschlüsselung:**  
   Mit \( K_{session} \) und einem einmaligen Nonce \( N \) wird der Klartext \( M \) zu Chiffretext \( C \) verschlüsselt, inklusive Authentifizierungstag \( T \).

4. **Schlüsselrotation:**  
   Periodisch wird aus dem aktuellen Schlüssel und einem Zeitstempel ein neuer Schlüssel erzeugt, um Sicherheit zu garantieren.
